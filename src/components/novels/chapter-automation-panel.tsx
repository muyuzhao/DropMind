...
import { AUTOMATION_TASK_STATUS_LABELS, automationTaskProgress, chapterBatchEnd, maxChapterBatchCount } from "@/modules/novels/automation-task";
...
type PendingConfirmation = { kind: "recover" } | { kind: "terminate" };
...
export function ChapterAutomationPanel({ novelId, nextWritableChapter, maxWritableChapterCount, newBatchStart, onImported, onManifestChange, onOpenChapter }: {
  novelId: string;
  nextWritableChapter: number | null;
  maxWritableChapterCount: number;
  ...
  const startChapter = nextWritableChapter ?? 60;
  const maxChapterCount = Math.min(maxChapterBatchCount(startChapter), Math.max(1, maxWritableChapterCount));
  const selectedChapterCount = Math.min(chapterCount, maxChapterCount);
  const endChapter = chapterBatchEnd(startChapter, selectedChapterCount);
  ...
  const canCreateNew = Boolean(run && nextWritableChapter !== null && !["pending", "running", "paused"].includes(run.manifest.status) && (run.manifest.status !== "completed" || run.manifest.nodes.every((node) => node.imported)));

  async function createRun() {
    if (busyAction) return;
    if (nextWritableChapter === null) { setMessage("第一卷第1–60章均已有正文"); return; }
    if (startChapter + selectedChapterCount - 1 > 60) { setMessage("生成范围不能超过第 60 章"); return; }
    setBusyAction("create");
    try {
      const result = await createChapterAutomationRunAction({ novelId, startChapter, chapterCount: selectedChapterCount });
      if (!result.ok) { setMessage(result.error); return; }
  ...
  async function confirmPendingAction() {
    ...
    if (pending.kind === "recover") await recoverInterrupted();
    else await control("terminate");
  }

  const confirmationTitle = confirmation?.kind === "recover" ? "确认终端已经中断？"
      : "终止这批正文任务？";
  const confirmationDescription = confirmation?.kind === "recover"
      ? "仅在 PowerShell 已返回提示符或窗口已经关闭时继续，运行中的章节会恢复为可重试状态。"
      : "任务不会再继续执行，已经生成的正文仍会保留在任务目录中。返回编辑不会停止任务。";

  const confirmationDialog = <WorkspaceConfirmDialog open={Boolean(confirmation)} title={confirmationTitle} description={confirmationDescription} confirmLabel={confirmation?.kind === "terminate" ? "终止任务" : "确认已中断"} danger={confirmation?.kind === "terminate"} busy={Boolean(busyAction)} onConfirm={() => void confirmPendingAction()} onClose={() => setConfirmation(null)} />;
  ...
  if (!run || creatingNew) return <section className="automation-panel">
    ...
    <p>自动从下一可写章节开始，只能连续追加，不能覆盖已有正文。每章独立调用一次 Codex，后一章读取前一章输出；全部完成后才会批量导入。</p>
    {nextWritableChapter === null ? <div className="automation-message">第一卷第1–60章均已有正文，没有可继续生成的章节。</div> : <>
    <div className="chapter-automation-form">
      <strong>起始章节：第 {startChapter} 章（自动接续）</strong>
      <label>连续生成<select value={selectedChapterCount} onChange={(event) => setChapterCount(Number(event.target.value))}>{Array.from({ length: maxChapterCount }, (_, index) => index + 1).map((count) => <option key={count} value={count}>{count} 章</option>)}</select></label>
  ...
  const visibleFailureReason = manifest.nodes.some((node) => node.status === "failed" || node.status === "stale") ? manifest.failureReason : null;
  const nextBatchStart = nextWritableChapter;
  ...
  return <section className="automation-panel">
    ...
    {visibleFailureReason && <div className="automation-error">{visibleFailureReason}</div>}
