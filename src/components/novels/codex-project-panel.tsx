...
export function CodexProjectPanel({ chapter, project, state, message, onSync, onPrepare, onImport, onRefresh, onCopyPath }: {
  ...
  </div><div className="codex-project-actions"><button type="button" onClick={onSync}>同步全部资料</button><button type="button" disabled={state.missing.length > 0} onClick={onPrepare}>准备任务并复制指令</button><button type="button" disabled={!state.fileExists} onClick={onImport}>{state.phase === "imported" ? "重新读取本章正文" : "预览并导入本章正文"}</button><button type="button" onClick={onRefresh}>刷新状态</button><button type="button" onClick={onCopyPath}>复制项目路径</button></div></section>;
}
